...
<body class="page-body">
					...
					<table class="table table-striped table-bordered" id="user">
						...
						<tbody>
							<?php if(is_array($allDaily)): foreach($allDaily as $key=>$vo): ?><tr>
								...
								<td>
									<?php if(isset($actions['TEAMDAILYLISTLOOK'])): ?><button class="btn btn-blue btn-xs"  onclick="window.location.href='<?php $str = $Think.INDEX_PATH_NAME.$actions['TEAMDAILYLISTLOOK']['url'];echo U($str,array('key'=>$actions['TEAMDAILYLISTLOOK']['key'],'id'=>urlsafe_b64encode(authcode($vo['id'],'ENCODE'))))?>'">查看</button><?php endif; ?>
								</td>
	...
	<script type="text/javascript" src="/efan/Public/Home/js/Daily/index.js"></script>
